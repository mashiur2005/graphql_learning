...
object ProductSearchRoutes {

  def route(context: ProductSearchContext)(implicit system: ActorSystem, ec: ExecutionContext): Route =
    path("graphql") {
      post {
              ...
              QueryParser.parse(query) match {
                case Success(queryAst) =>
                  val result: Future[Json] = Executor
                    .execute(
                      schema = ProductSchemaDefinition.schema,
                      queryAst = queryAst,
                      userContext = context,
