...
- **`app/src/main/scala/com/example/App.scala`** is the original Gradle-init-generated "Hello, world!" sample and is unused by the running application (`application.mainClass` in `app/build.gradle` points to `com.example.GraphQLServer`). `AppSuite.scala` tests only that leftover file.
